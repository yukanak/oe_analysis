...
# input raw bias/fb in ADU, only for 1 det
# output biascalib, fbcalib with shift, rnti
def get_LC(bias, fb, calib, fitrange = None, row = None, col = None, out_path = None, flip = 1, DCflag=0):
	...
	ceff2  = np.polyfit(bias_fit*calib["BIAS_CAL"][0],-1*flip*fb_fit*calib["FB_CAL"][0],2)
	curve = np.poly1d(ceff2)

	# pl.plot(bias_fit*calib["BIAS_CAL"][0],-1*flip*fb_fit*calib["FB_CAL"][0])
	# pl.plot(bias_fit*calib["BIAS_CAL"][0], curve(bias_fit*calib["BIAS_CAL"][0]))
	# pl.show()
	...
	return biascalib, fbcalib, RR, ceff_sc[0], shift_h, ceff2[0]
...
def get_PR_Ti(biascalib, fbcalib, calib, rnti, rnpsat, bound, ksc = None, row = None, col = None, out_path = None, flip = 1):
	...
	if len(ind):
		rr_single = rr[ind][::-1]
		mask = (rr_single>(rnpsat-0.02)) & (rr_single<(rnpsat+0.02))
		try:
...
def get_PR(biascalib, fbcalib, calib, rnti, rnpsat, row = None, col = None, out_path = None, flip = 1, reasonableInorm = 30.0e-6):
	...
	Inorm = np.where((fbcalib<1.1*biascalib*calib["R_SH"]/(calib["R_SH"]+rnti))
			& (fbcalib>0.9*biascalib*calib["R_SH"]/(calib["R_SH"]+rnti)), fbcalib, float('nan'))
	# mask = np.where((np.logical_and(rr>5,rr<=15))
	# 		& (np.logical_and(pp>0,pp<0.5e-11)))
	#print('Inorm=', Inorm)
	# zeromask=np.where(pp>0.5e-12)
	# pp=pp[zeromask]
	# rr=rr[zeromask]
	...
	if len(ind):
		...
		#pl.plot(rr, pp, c='r')
		#pl.axvline(rnti*1.2, c='k')
		#pl.axvline(rnti*1.1, c='k')
		idx=np.where(np.logical_and((rr>=0.95*rnpsat),(rr<=1.05*rnpsat)))[0]
		#print(pp[idx])
		psat=np.nanmean(pp[idx])
		#pl.axhline(y=psat, c='b')
		#pl.show()
		#sys.exit()
		if psat < 0 or psat > 1000e-12:
			psat = float('nan')
	...
	if doFigure:
		...
		pl.plot(rr*1.00e3,pp*1.00e12, color='k', linewidth=2)
                        ...
                        #pl.ylim(0,10)
                #print(out_path)
		#fn = out_path+'single_pr_row'+row+'_col'+col+'.png'
		pl.axvline(x=rnti*1.0e3, color='r', linestyle='--', alpha=0.5)
		pl.axhline(y=psat*1.0e12, color='r', linestyle='--', alpha=0.5)
		pl.grid()
		pl.tight_layout()
