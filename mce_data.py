...
class SmallMCEFile:
    ...
    def Read(self, count=None, start=0,
        ...
        if len(kwargs) > 0:
            print(kwargs)
            sys.exit()
            raise TypeError("%s: got an expected keyword argument")# '%s'" %(sys._getframe().f_code.co_name, kwargs.keys()[0]))
...
class MCERunfile:
    ...
    def FindRunfile(filename, follow_links=None):
        ...
        for i in range(len(filename.split('/'))-1):
            print(pathtorun)
            pathtorun+=filename.split('/')[i]+'/'
        general_runfile = pathtorun+'general_runfile.run'
        if os.path.exists(runfile):
            return runfile
        runfile = os.path.splitext(filename)[0] + '.run'
        if os.path.exists(runfile):
            return runfile
        elif os.path.exists(general_runfile):
            return general_runfile
